...
def meters_to_dx(m): return m / (111_320.0 * math.cos(AVG_LAT_RAD)) if IS_GEOGRAPHIC else m
def meters_to_dy(m): return m / 110_540.0 if IS_GEOGRAPHIC else m
def dx_map_to_m(dx): return dx * 111_320.0 * math.cos(AVG_LAT_RAD) if IS_GEOGRAPHIC else dx
def dy_map_to_m(dy): return dy * 110_540.0 if IS_GEOGRAPHIC else dy
...
def park_chart():
    ...
    try:
        ...
        # --- NEW: wind-aware banded hazard (red / yellow / green) ---
        hazard_gui.add_banded_hazard(
            fig,
            HAZARDS,
            x_domain,
            y_domain,
